...
def _absolute_yad2_url(value: str) -> str:
    value = unescape(value).strip()
    if value.startswith("https://www.yad2.co.il/"):
        return value
    if value.startswith("https://yad2.co.il/"):
        return value.replace("https://yad2.co.il/", "https://www.yad2.co.il/", 1)
    if value.startswith("/"):
        return "https://www.yad2.co.il" + value
    return ""
...
def _yad2_dict_to_listing(item: dict[str, Any], city: str, search_url: str) -> Listing | None:
    url = ""
    for key in ("url", "link", "href", "itemUrl", "item_url", "adUrl", "ad_url"):
        raw = item.get(key)
        if isinstance(raw, str) and ("/realestate/item/" in raw or "/realestate/rent/" in raw):
            url = _absolute_yad2_url(raw)
            if url:
                break
    ...
    # Avoid treating the search page itself as an item.
    if "?" in url and "/realestate/rent/" in url and "/realestate/item/" not in url:
        return None

    title_parts: list[str] = []
    text_parts: list[str] = []
    for key in ("title", "address", "street", "neighborhood", "city", "propertyType", "property_type"):
        value = item.get(key)
        if isinstance(value, (str, int, float)) and str(value).strip():
            title_parts.append(str(value).strip())

    for key in ("description", "text", "subtitle", "row_1", "row_2", "info_text"):
        value = item.get(key)
        if isinstance(value, (str, int, float)) and str(value).strip():
    ...
    if isinstance(price, (str, int, float)) and str(price).strip():
        text_parts.append(f"מחיר {price} ₪")

    rooms = item.get("rooms") or item.get("roomCount") or item.get("room_count")
    if isinstance(rooms, (str, int, float)) and str(rooms).strip():
        text_parts.append(f"{rooms} חדרים")

    title = " | ".join(dict.fromkeys(title_parts)) or "מודעת יד2"
    body = " | ".join(dict.fromkeys(text_parts))
    if not body:
        body = f"מודעת יד2 בחיפוש {city}"

    return Listing("Yad2", city, title, body, url, None)


def run_yad2_search(search_url: str, city: str) -> list[Listing]:
    """Fetch a filtered Yad2 search page. Failure here never stops Facebook monitoring."""
    headers = {
        "User-Agent": (
    ...
    if not response.ok:
        raise RuntimeError(f"Yad2 HTTP {response.status_code}: {response.text[:300]}")

    html_text = response.text
    found: dict[str, Listing] = {}

    # First path: direct listing links rendered in the page HTML.
    link_re = re.compile(
        r"href=[\"']([^\"']*(?:/realestate/item/)[^\"'#?]*)[^\"']*[\"']",
        re.I,
    )
    for match in link_re.finditer(html_text):
        url = _absolute_yad2_url(match.group(1))
        if not url:
            continue
        start = max(0, match.start() - 1800)
        end = min(len(html_text), match.end() + 1800)
        card_text = _strip_html(html_text[start:end])
        found[url] = Listing("Yad2", city, "מודעת יד2", card_text, url, None)

    # Second path: structured JSON embedded by the site (Next.js / JSON state / JSON-LD).
    for script_body in re.findall(r"<script[^>]*>(.*?)</script>", html_text, flags=re.I | re.S):
        body = unescape(script_body).strip()
        ...
        for item in _walk_json(payload):
            listing = _yad2_dict_to_listing(item, city, search_url)
            if listing:
                found.setdefault(listing.url, listing)

    print(f"[Yad2] city={city} listings={len(found)} url={search_url}")
...
def evaluate_listing(listing: Listing, cfg: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    # Yad2 search URLs already enforce the room range. On Facebook we require explicit detection.
    if rooms is not None and not (rmin <= rooms <= rmax):
        return False, {"reason": f"חדרים לא בטווח {rmin}-{rmax}"}
    if rooms is None and not is_yad2:
        return False, {"reason": f"חדרים לא בטווח {rmin}-{rmax}"}

    price = extract_price(text)
    pmax = int(cfg.get("price_max", 7500))
    if price is None and not bool(cfg.get("allow_missing_price", True)) and not is_yad2:
        return False, {"reason": "לא נמצא מחיר"}
    if price is not None and price > pmax:
        return False, {"reason": f"מחיר מעל {pmax}"}

    no_parking = contains_any(text, [
        "ללא חניה", "אין חניה", "חניה ברחוב", "חנייה ברחוב",
        "street parking", "nearby parking"
    ])
    if no_parking:
        return False, {"reason": "אין חניה פרטית"}

    # Yad2 links already contain parking=1; Facebook must mention parking in the post.
    has_parking = is_yad2 or contains_any(text, [
        "חניה פרטית", "חנייה פרטית", "חניה בטאבו", "חנייה בטאבו",
        "חניה צמודה", "חנייה צמודה", "חניה לדירה", "חנייה לדירה",
        "חניה", "חנייה", "חניות", "2 חניות", "שתי חניות",
        "private parking", "assigned parking"
    ])
    # If parking is not mentioned, keep the listing so we don't miss it.
    # Explicit "no parking" is still rejected above.

    no_elevator = contains_any(text, [
        "אין מעלית", "ללא מעלית", "בלי מעלית", "no elevator", "no lift"
    ])
    if no_elevator:
        return False, {"reason": "אין מעלית"}

    # Yad2 links already contain elevator=1.
    has_elevator = is_yad2 or contains_any(text, ["מעלית", "elevator", "lift"])
    # If elevator is not mentioned, keep the listing so we don't miss it.
    # Explicit "no elevator" is still rejected above.

    street, street_ok = extract_street(raw)
    # IMPORTANT: Yad2 Holon URL is already neighborhood=793 (Agrobank), so do NOT apply street names.
    if not is_yad2 and cfg.get("require_agrobank", False) and not street_ok:
        return False, {"reason": "לא זוהה רחוב מאגרובנק"}

    no_balcony = contains_any(text, ["אין מרפסת", "ללא מרפסת", "בלי מרפסת"])
    if no_balcony and cfg.get("yad2_balcony_required", False) and is_yad2:
        return False, {"reason": "אין מרפסת"}
    # Holon Yad2 URL includes balcony=1, so trust it even if card text omits the word.
    has_balcony = (is_yad2 and cfg.get("yad2_balcony_required", False)) or contains_any(
        text, ["מרפסת", "balcony", "בלקון"]
    )
    if no_balcony:
        balcony_status = "❌ אין"
    elif has_balcony:
        ...
        balcony_status = "❓ לא צוין"

    has_mamad = contains_any(text, [
        'ממ"ד', "ממד", "ממ״ד", "חדר ממד", "חדר ממ\"ד", "safe room", "protected room"
    ])
    has_mamak = contains_any(text, [
        'ממ"ק', "ממק", "ממ״ק", "מרחב מוגן קומתי", "ממ\"ק קומתי", "ממק קומתי"
    ])
    has_shelter = contains_any(text, [
        "מקלט", "מקלט בבניין", "מקלט בבנין", "מקלט משותף", "מקלט קרוב"
    ])

    top_floor = (
        contains_any(text, ["קומה אחרונה", "קומה עליונה"])
        or bool(re.search(r"\b(\d+)\s*(?:מתוך|/)\s*\1\b", text))
    )
    ...
    return True, {
        "rooms": rooms if rooms is not None else "לפי פילטר יד2",
        "price": price,
        "street": street or ("אגרובנק (לפי פילטר יד2)" if is_yad2 and listing.city_rule == "חולון" else "לא זוהה"),
        "parking_status": "✅ יש" if has_parking else "❓ לא צוין",
        "elevator_status": "✅ יש" if has_elevator else "❓ לא צוין",
        "balcony_status": balcony_status,
        "mamad": has_mamad,
        "mamak": has_mamak,
        "shelter": has_shelter,
        "top_floor": top_floor,
...
def build_email(matches: list[tuple[Listing, dict[str, Any]]], issues: list[str]) -> tuple[str, str]:
    ...
    for i, (listing, info) in enumerate(matches, 1):
        mamad_status = "✅ יש" if info.get("mamad") else "❓ לא צוין"
        mamak_status = "✅ יש" if info.get("mamak") else "❓ לא צוין"
        shelter_status = "✅ יש" if info.get("shelter") else "❓ לא צוין"
        top_floor_status = "⚠️ כן" if info.get("top_floor") else "❓ לא זוהה"
        price_text = f"{info.get('price')} ₪" if info.get("price") is not None else "לא צוין"

        parts.append(
            f"{i}. 🏠 {listing.title or 'מודעה ללא כותרת'}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🛏️ חדרים: {info.get('rooms')}\n"
            f"💰 מחיר: {price_text}\n"
            f"📍 עיר: {listing.city_rule}\n"
            f"🌐 מקור: {listing.source}\n"
            f"🛣️ רחוב: {info.get('street')}\n\n"
            f"🔎 סיכום מהיר:\n"
            f"🅿️ חניה: {info.get('parking_status')}\n"
            f"🛗 מעלית: {info.get('elevator_status')}\n"
            f"🌿 מרפסת: {info.get('balcony_status')}\n"
            f"🛡️ ממ״ד: {mamad_status}\n"
            f"🏢 ממ״ק קומתי: {mamak_status}\n"
            f"🛡️ מקלט: {shelter_status}\n"
            f"🏢 קומה אחרונה: {top_floor_status}\n"
            f"⭐ ציון התאמה: {info.get('score', 0)}/10\n\n"
...
def build_run_report_email(
    matches: list[tuple[Listing, dict[str, Any]]],
    issues: list[str],
    stats: dict[str, int],
) -> tuple[str, str]:
    subject, details = build_email(matches, issues)

    if matches:
        subject = f"✅ דוח ניטור: {len(matches)} התאמות חדשות"
    elif issues:
        subject = "⚠️ דוח ניטור: הסתיים עם בעיות גישה"
    else:
        subject = "✅ דוח ניטור: אין התאמות חדשות"

    summary = (
        "✅ הסריקה הסתיימה\n\n"
        "Facebook:\n"
        f"• קבוצות שנבדקו: {stats.get('facebook_attempted', 0)}\n"
        f"• קבוצות שהחזירו תשובה: {stats.get('facebook_success', 0)}\n"
        f"• פוסטים שנמשכו: {stats.get('facebook_items', 0)}\n\n"
        "Yad2:\n"
        f"• חיפושים שנבדקו: {stats.get('yad2_attempted', 0)}\n"
        f"• חיפושים שהחזירו תשובה: {stats.get('yad2_success', 0)}\n"
        f"• מודעות שנמשכו: {stats.get('yad2_items', 0)}\n\n"
        f"🏠 התאמות חדשות: {len(matches)}\n"
        f"⚠️ בעיות גישה: {len(issues)}\n"
        "━━━━━━━━━━━━━━━━━━\n\n"
    )

    if not matches and not issues:
        details = "לא נמצאו מודעות חדשות שעברו את כל הסינונים."
...
def build_rules(cfg: dict[str, Any]) -> list[SearchRule]:
    ...
    for city, city_cfg in cfg.get("cities", {}).items():
        # TEMP: Ramat Gan is kept in config.json but disabled for now.
        if city == "רמת גן":
            continue
...
def main() -> int:
    ...
    # Per-run status counters used only for the email report.
    stats = {
        "facebook_attempted": 0,
        "facebook_success": 0,
        "facebook_items": 0,
        "yad2_attempted": 0,
        "yad2_success": 0,
    ...
    for rule in rules:
        for url in rule.urls:
            ...
            try:
                ...
                stats["facebook_items"] += len(items)
    ...
    # Yad2 is independent of the Facebook/Bright Data flow. If Yad2 fails,
    # Facebook monitoring still completes normally.
    for city, city_cfg in cfg.get("cities", {}).items():
        # TEMP: Ramat Gan is kept in config.json but disabled for now.
        if city == "רמת גן":
            continue
        for search_url in city_cfg.get("yad2_search_urls", []):
            search_url = str(search_url).strip()
            if not search_url:
                continue
            stats["yad2_attempted"] += 1
            print(f"[Monitor] fetching Yad2 city={city} url={search_url}")
            try:
                listings = run_yad2_search(search_url, city)
                stats["yad2_success"] += 1
                stats["yad2_items"] += len(listings)
                print(f"[Monitor] received Yad2 city={city} items={len(listings)}")
                for listing in listings:
                    raw.append((listing, city_cfg))
            except Exception as exc:
                issue = f"Yad2 {city}: {exc}"
                print(f"[Monitor][issue] {issue}", file=sys.stderr)
                issues.append(issue)
    ...
    subject, body = build_run_report_email(matches, issues, stats)
    send_email(subject, body, cfg["recipient_email"])
    print(
        f"Email sent. Matches={len(matches)}, issues={len(issues)}, "
        f"facebook_groups={stats['facebook_attempted']}, "
        f"facebook_items={stats['facebook_items']}, "
        f"yad2_searches={stats['yad2_attempted']}, "
        f"yad2_items={stats['yad2_items']}"
    )
