...
def main() -> int:
    ...
    print(f"[Monitor] mode={mode} rules={len(rules)} lookback_hours={lookback} max_posts={max_posts}")
    ...
    if not initial_backfill_done:
        # First run with this version: send ALL posts returned for the previous lookback window.
        # Do not apply apartment filters in this one email.
        initial_listings = [listing for listing, _ in unique_raw]
