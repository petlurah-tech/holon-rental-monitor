...
def evaluate_listing(listing: Listing, cfg: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    has_parking = contains_any(text, [
        "חניה פרטית", "חנייה פרטית",
        "חניה בטאבו", "חנייה בטאבו",
        "חניה צמודה", "חנייה צמודה",
        "חניה לדירה", "חנייה לדירה",
        "חניה", "חנייה", "חניות",
        "2 חניות", "שתי חניות",
    ...
    has_mamad = contains_any(text, [
        'ממ"ד', "ממד", "ממ״ד", "חדר ממד", "חדר ממ\"ד",
        "safe room", "protected room"
    ])
    has_mamak = contains_any(text, [
        'ממ"ק', "ממק", "ממ״ק",
        "מרחב מוגן קומתי", "ממ\"ק קומתי", "ממק קומתי"
    ])
    has_shelter = contains_any(text, [
        "מקלט", "מקלט בבניין", "מקלט בבנין",
        "מקלט משותף", "מקלט קרוב"
    ])
