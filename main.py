...
def run_brightdata_group(group_url: str, start_date: str, end_date: str, num_of_posts: int) -> list[dict[str, Any]]:
    ...
    if not response.ok:
        ...
        print(f"[BrightData][HTTP {response.status_code}] url={group_url} error={error_data}", file=sys.stderr)
    ...
    if isinstance(data, dict):
        ...
        if error:
            print(f"[BrightData][API error] url={group_url} error={error}", file=sys.stderr)
