...
def normalize_items(items: list[dict[str, Any]], source: str) -> list[Listing]:
    ...
    for item in items:
        title = first_text(item, ["title", "name", "headline"])
        text = first_text(item, ["text", "description", "postText", "content", "body"])
        url = first_text(item, ["url", "postUrl", "link", "listingUrl"])
        published = None
        for key in ["publishedAt", "date", "timestamp", "createdAt", "publicationDate"]:
            if item.get(key):
...
def main() -> int:
    ...
    fb_actor = os.getenv("APIFY_FB_ACTOR_ID", "").strip()
    yd_actor = os.getenv("APIFY_YAD2_ACTOR_ID", "").strip()

    if fb_actor:
        try:
            items = run_apify_actor(fb_actor, {"startUrls": [{"url": u} for u in cfg["facebook_group_urls"]]})
            raw_listings.extend(normalize_items(items, "Facebook"))
        except Exception as exc:
            access_issues.append(f"Facebook: {exc}")
    else:
        access_issues.append("Facebook: APIFY_FB_ACTOR_ID לא הוגדר")

    if yd_actor and cfg.get("yad2_search_url"):
        try:
            items = run_apify_actor(yd_actor, {"startUrls": [{"url": cfg["yad2_search_url"]}]})
            raw_listings.extend(normalize_items(items, "Yad2"))
        except Exception as exc:
            access_issues.append(f"Yad2: {exc}")
    else:
        access_issues.append("Yad2: חסר קישור חיפוש או APIFY_YAD2_ACTOR_ID")
    ...
    if fresh_matches or access_issues:
        ...
        if not fresh_matches:
            subject = "דוח ניטור: בעיות גישה למקורות"
